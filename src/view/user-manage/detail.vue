          ...
          <div class="group-level" v-show="data.optionSelect.roles.length > 0">
            ...
            <div class="level-button">
              <div v-for="(item, index) in data.optionSelect.roles" :key="index">
                ...
                <button v-else type="button" class="button-roles" v-bind:class="item.check ? 'active' : ''" @click="item.check = !item.check" :disabled="item.id == 6 ? !(rule.user1 || rule.user5) : false">
                  {{item.desc}}
                </button>
              </div>
            </div>
