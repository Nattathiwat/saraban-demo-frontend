      ...
      <div class="detail">
        <div class="group-head">
          <div class="group-first">
            ...
            <div class="name">
              {{ edit ? "แก้ไขหนังสือรับเข้า" : "สร้างหนังสือรับเข้า" }}
              <label>{{ data.receive_document_number }}</label>
            </div>
          </div>
        </div>
        <div class="line"></div>
        <Form @submit="on_submit" @invalid-submit="onInvalidSubmit">
          <div class="group-detail">
            ...
            <div class="group-between">
              ...
              <div class="group-between">
                <div class="group-input left">
                  ...
                  <cpn-datepicker
                    v-model="data.receive_date"
                    name="receive_date"
                    rules="required"
                    :disabled="
                      (edit && data.book_type != 0) || data.status == 2 || data.status == 3
                    "
                    placeholder="กรุณาระบุ"
                  />
                </div>
            ...
            <div class="group-input left">
              ...
              <cpn-textArea v-model="data.book_desc" name="book_desc" rows="1" />
            </div>
            <div class="group-input">
                ...
                >
                  <div class="d-flex mb-3">
                    <div class="group-input-file">
                      <button
                        type="button"
                        :class="edit && data.book_type != 0 ? 'none-pointer' : ''"
                        class="button-file"
                        @click="edit && data.book_type != 0 ? '' : upload_file(`main_docs${index}`)"
                      >
                      ...
                      <div
                        :class="edit && data.book_type != 0 ? 'text disabled' : 'text pointer'"
                        @click="edit && data.book_type != 0 ? '' : upload_file(`main_docs${index}`)"
                      >
                        แนบเอกสาร
                      </div>
              ...
              <div class="group-input">
                <div class="group-input d-flex align-items-center">
                  ...
                  <button
                    ...
                    :disabled="edit && data.book_type != 0"
                    @click="upload_file('fileAttachment')"
                  >
                    <div class="group-image">
                      <img
                        src="@/assets/images/icon/plus-circle-duotone.svg"
                  ...
                  <div class="d-flex mb-3">
                    <div class="group-input-file">
                      <button
                        ...
                        class="button-file"
                        @click="
                          edit && data.book_type != 0 ? '' : upload_file(`attachments${index}`)
                        "
                      >
                        <span :class="item.filename ? '' : 'no-data'">
              ...
              >
                <div class="detail-head">
                  ...
                  <div
                    class="topic"
                    :class="
                      item.bookaction_name == 'ความเห็นคำสั่ง'
                        ? 'blue'
                        : item.bookaction_name == 'แก้ไขหนังสือ'
                        ? 'yellow'
                        : 'green'
                    "
                  >
                  ...
                  <div class="date">วันที่ {{ item.create_date }}</div>
                  <div class="time">
                    <i class="bi bi-clock icon-size"></i>
                    {{ item.create_time }}
                  </div>
                ...
                <ul
                  class="detail-list"
                  v-for="(item2, index2) in item.booking_remarks"
                  :key="index2"
                >
                  <button
                  ...
                  <li>
                    ...
                    <div
                      ...
                      v-if="item2.signature_img && item.bookaction_name == 'ความเห็นคำสั่ง'"
                    >
...
export default {
  ...
  methods: {
    historyClick(data) {
      ...
      this.axios
        .get(`/booking-receive/${this.$route.params.id}/history`, {
          params: {
    ...
    delete_click() {
      ...
      this.modalAlert = {
        ...
        afterPressAgree() {
          ...
          _this.axios
            .delete(`/booking-receive/${_this.$route.params.id}`, {
              params: {
    ...
    on_submit() {
      ...
      this.modalAlert = {
        ...
        type: "confirm",
        title: `คุณยืนยันการ${
          this.flagSave == 1 ? "บันทึกแบบร่าง" : this.flagSave == 3 ? "บันทึก" : "บันทึกและส่งต่อ"
        }หรือไม่`,
        confirm: true,
        msgSuccess: true,
        afterPressAgree() {
    ...
    call_api_save(fileMain_docs, fileAttachments) {
      ...
      this.axios[this.edit ? "put" : "post"](
        `/booking-receive${this.edit ? "/" + this.$route.params.id : ""}`,
        dataSave
      )
        .then(() => {
          this.showLoading = false;
    ...
    api_detail() {
      this.showLoading = true;
      this.axios
        .get(`/booking-receive/${this.$route.params.id}`, {
          params: {
            book_type: this.$route.query.book_type,
        ...
        .then((response) => {
          ...
          if (this.data.contracts?.length < 1 || !this.data.contracts)
            this.data.contracts = [
              {
                department_id: "",
                receive_type: "",
          ...
          this.data.book_type = this.data.book_type
            ? this.data.book_type
            : this.$route.query.book_type;
        })
        .catch((error) => {
          this.showLoading = false;
          this.modalAlert = {
    ...
    submitClick() {
      ...
      this.modalAlert = {
        ...
        afterPressAgree() {
          ...
          _this.showLoading = true;
          _this.axios
            .put(`/booking-receive/${_this.$route.params.id}`, groupdata)
            .then(() => {
              _this.showLoading = false;
              _this.modalAlert = {
                showModal: true,
