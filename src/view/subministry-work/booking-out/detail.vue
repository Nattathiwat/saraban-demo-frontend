      ...
      <div class="detail">
        ...
        <Form @submit="on_submit" @invalid-submit="onInvalidSubmit">
          <div class="group-detail">
            ...
            <div class="group-input d-flex align-items-center">
              ...
              <button type="button" class="add-booking-out" @click="add_booking_refers()">
                <div class="group-image">
                  <img
                    src="@/assets/images/icon/plus-circle-duotone.svg"
                    alt=""
                    class="icon-plus"
                  />
                  เพิ่มเอกสารอ้างอิง
            ...
            <div
              class="group-between"
              v-for="(item, index) in data.booking_refers"
              :key="index"
              v-show="item.flag != 'delete'"
            >
              <div class="group-input left">
                <cpn-input
                  v-model="item.receive_document_number"
              ...
              <div class="group-input d-flex">
                ...
                <button
                  type="button"
                  @click="delete_booking_refers(item, index)"
                  class="button-delete ms-3"
                >
                  <img
                    src="@/assets/images/icon/trash-alt-duotone.svg"
                    alt=""
                    class="image-trash pointer"
                  />
                </button>
              </div>
            </div>
            <div class="group-input">
              ...
              <cpn-textArea v-model="data.subject" name="subject" rules="required" rows="1" />
            </div>
          </div>
          <div class="line"></div>
          <div class="d-flex justify-content-end">
            ...
            <button
              type="button"
              class="add-register"
              :disabled="!data.book_type_id || !data.secret_id || !data.speed_id"
              @click="add_booking_register_details()"
            >
          ...
          <div
            class="group-detail-2"
            v-for="(item, index) in data.booking_register_details"
            :key="index"
            v-show="item.flag != 'delete'"
          >
            <div class="d-flex">
              ...
              <div class="col-center">
                <div class="row">
                  <div class="col-lg-auto col-md-auto mb-3">
                    <span class="span">การออกเลข : {{ item.book_out_num_type_name }}</span
                    ><span>รูปแบบการส่ง : {{ item.send_method_name }}</span>
                  </div>
                  <div class="group-date col-lg-auto col-md-auto mb-3">
                    <div class="name">ลงวันที่ :</div>
                    <cpn-datepicker
                      v-model="item.regis_date"
              ...
              <div class="col-end">
                <div class="d-flex justify-content-end">
                  ...
                  <button
                    type="button"
                    class="del-department"
                    @click="delete_booking_register_details(item, index)"
                  >
                    <div class="group-image">
            ...
            <div class="group-all-file">
              <div class="name">เอกสารใช้ร่วมกัน</div>
              <div class="group-input-file">
                ...
                <input
                  type="file"
                  @change="
                    file_booking_register_details_change(`main_docs${index}`, index, 'main_docs')
                  "
                  :name="`main_docs${index}`"
                  style="display: none"
              ...
              <button
                ...
                @click="
                  download_file({
                    filename: item.main_filename,
                    type: item.main_type,
                    filepath: item.main_filepath,
                    link: item.main_link,
                  })
                "
                class="button-eye"
              ...
              <div class="group-input-file">
                <button
                  type="button"
                  class="button-file"
                  @click="upload_file(`dupplicate_copy${index}`)"
                >
                  <span :class="item.attach_filename ? '' : 'no-data'">
                    {{ item.attach_filename ? item.attach_filename : "สิ่งที่แนบมาด้วย" }}
                  </span>
                </button>
                <div class="text pointer" @click="upload_file(`dupplicate_copy${index}`)">
                  แนบเอกสาร
                </div>
                <input
                  type="file"
                  @change="
                    file_booking_register_details_change(
                      `dupplicate_copy${index}`,
                      index,
                      'dupplicate_copy'
                    )
                  "
                  :name="`dupplicate_copy${index}`"
                  style="display: none"
                />
              </div>
              <button
                ...
                @click="
                  download_file({
                    filename: item.attach_filename,
                    type: item.attach_type,
                    filepath: item.attach_filepath,
                    link: item.attach_link,
                  })
                "
                class="button-eye"
              >
            ...
            <div
              class="detail-sub"
              v-for="(item2, index2) in item.booking_registers"
              :key="index2"
            ...
            >
              <div class="group-between">
                ...
                <div class="group-input">
                  ...
                  <cpn-input v-model="item2.greeting" :name="`greeting${index}${index2}`" />
                </div>
              </div>
              <div class="group-input">
                <div class="name">หน่วยงานปลายทาง</div>
                <cpn-autoComplete
                  v-model="item2.department_dest_id"
                  ...
                  <div class="d-flex">
                    <div class="group-input-file">
                      <button
                        type="button"
                        class="button-file"
                        @click="upload_file(`main_docs${index}${index2}`)"
                      >
                        <span :class="item2.main_filename ? '' : 'no-data'">
                          {{ item2.main_filename ? item2.main_filename : "หนังสือต้นเรื่อง" }}
                        </span>
                      </button>
                      <div class="text pointer" @click="upload_file(`main_docs${index}${index2}`)">
                        แนบเอกสาร
                      </div>
                      <input
                        type="file"
                        @change="
                          file_booking_registers_change(
                            `main_docs${index}${index2}`,
                            index,
                            index2,
                            'main_docs'
                          )
                        "
                        :name="`main_docs${index}${index2}`"
                        style="display: none"
                        accept="application/pdf"
                      />
                    </div>
                    <button
                      ...
                      @click="
                        download_file({
                          filename: item2.main_filename,
                          type: item2.main_type,
                          filepath: item2.main_filepath,
                          link: item2.main_link,
                        })
                      "
                      class="button-eye"
                    >
                  ...
                  <div class="d-flex mt-3">
                    <div class="group-input-file">
                      <button
                        type="button"
                        class="button-file"
                        @click="upload_file(`dupplicate_copy${index}${index2}`)"
                      >
                      ...
                      <input
                        ...
                        @change="
                          file_booking_registers_change(
                            `dupplicate_copy${index}${index2}`,
                            index,
                            index2,
                            'dupplicate_copy'
                          )
                        "
                        :name="`dupplicate_copy${index}${index2}`"
                        style="display: none"
                      />
                    </div>
                    <button
                      ...
                      @click="
                        download_file({
                          filename: item2.attach_filename,
                          type: item2.attach_type,
                          filepath: item2.attach_filepath,
                          link: item2.attach_link,
                        })
                      "
                      class="button-eye"
                    >
                    ...
                    <button
                      ...
                      class="del-department-3"
                      @click="
                        (item2.attach_filename = ''),
                          (item2.attach_filepath = ''),
                          (item2.attach_filesize = '')
                      "
                    >
                      <img
                        src="@/assets/images/icon/trash-alt-duotone.svg"
              ...
              <div class="name mt-3 ms-2 required" v-show="item2.send_style_desc">
                วิธีการส่ง : {{ item2.send_style_desc }}
              </div>
              <div class="text-end">
                <button
                  type="button"
                  class="del-department-2"
                  @click="delete_booking_registers(item2, item, index2)"
                >
                  <i class="bi bi-trash-fill image-trash pointer"></i>
                </button>
              </div>
          ...
          <div
            v-if="data.booking_register_details.filter((el) => el.flag != 'delete').length > 0"
            class="line"
          ></div>
          <div
            v-if="data.booking_register_details.filter((el) => el.flag != 'delete').length > 0"
          ...
          >
            <div class="group-input">
              ...
              <cpn-input-tags v-model="data.tag" name="tag" />
            </div>
            <div class="group-input d-flex align-items-center mt-4">
              ...
              <input
                type="file"
                multiple
                @change="file_attachment_add_change(`fileAttachment`)"
                :name="`fileAttachment`"
                style="display: none"
              />
            </div>
                ...
                <div class="d-flex mb-3 group-input-file-all">
                  <div class="group-input-file">
                    <button
                      type="button"
                      class="button-file"
                      @click="upload_file(`fileAttachment${index}`)"
                    >
                      <span :class="item.filename ? '' : 'no-data'">
                        {{ item.filename ? item.filename : "สิ่งที่แนบมาด้วย" }}
                      </span>
                    </button>
                    <div class="text pointer" @click="upload_file(`fileAttachment${index}`)">
                      แนบเอกสาร
                    </div>
                    <input
                      type="file"
                      @change="file_attachment_change(`fileAttachment${index}`, item)"
                      :name="`fileAttachment${index}`"
                      style="display: none"
                    />
                  </div>
                  <button type="button" @click="download_file(item)" class="button-eye">
                    <i class="bi bi-eye icon-eye"></i>
                  </button>
                  <button
                    type="button"
                    class="del-department-3"
                    @click="delete_attachments(item, index)"
                  >
                    <img
                      src="@/assets/images/icon/trash-alt-duotone.svg"
                      alt=""
          ...
          >
            ...
            <div class="group-input left">
              <div class="name d-flex justify-content-between">
                ...
                <div>
                  <input
                    type="file"
                    @change="file_set_change('sendTo', 0, 'sendTo')"
                    name="sendTo"
                    style="display: none"
                  />
                  <button
                    v-if="!data.sendToFile?.filename"
                    type="button"
                    class="button-con pointer"
                    @click="upload_file('sendTo')"
                  >
                    <img
                      src="@/assets/images/icon/paperclip-solid.svg"
                      alt=""
              ...
              <cpn-textArea v-model="data.comment" name="comment" rows="3" />
            </div>
            <div class="group-between">
              <div class="group-input">
            ...
            <div
              class="group-add"
              v-for="(item, index) in data.booking_follows"
              :key="index"
              v-show="item.flag != 'delete'"
            >
              <div class="d-flex justify-content-between">
                ...
                <img
                  @click="delete_booking_follows(item, index)"
                  src="@/assets/images/icon/trash-alt-duotone.svg"
                  alt=""
                  class="image-trash pointer"
                />
              ...
              <div class="d-flex ms-2 mt-2">
                ...
                <div class="name ms-5">
                  รูปแบบการดำเนินการ : {{ item?.process_type_name || "-" }}
                </div>
          ...
          <div class="group-footer">
            ...
            <div
              class="footer-right"
              v-show="data.booking_register_details.filter((el) => el.flag != 'delete').length > 0"
            >
              <button
                type="submit"
                class="button-success button-save"
                @click="flagSave = 3"
                v-show="edit"
              >
                <img
              ...
              <button
                ...
                @click="flagSave = 5"
                v-if="button_send_bookout && !data.is_show_send_style_button"
              >
                <img
                  src="~@/assets/images/icon/check-circle-duotone.svg"
                  alt="times-circle"
              ...
              <button
                ...
                class="button-success"
                @click="flagSave = 2"
                :disabled="data.sendTo.length < 1"
                v-if="!button_send_bookout"
              >
                <img
                  src="~@/assets/images/icon/check-circle-duotone.svg"
                  alt="times-circle"
          ...
          <div class="modal-size" ref="modalRegiterref">
            <Form @submit="on_submit_modal" @invalid-submit="onInvalidSubmit">
              ...
              <div class="modal-detail">
                <div
                  class="message"
                  v-for="(item, index) in modalRegiter.booking_register_details"
                  :key="index"
                >
                  <div class="d-flex justify-content-between">
                    <div class="set">
                      ชุดที่ #{{ index + 1 + data.booking_register_details.length }}
                    </div>
                    <button
                      type="button"
                      @click="
                        modalRegiter.booking_register_details.length > 1
                          ? modalRegiter.booking_register_details.splice(index, 1)
                          : ''
                      "
                      class="button-delete ms-3"
                    >
                  ...
                  <div class="detail-sub">
                    <div class="group-between">
                      ...
                      <div class="group-input right-width">
                        ...
                        <cpn-select
                          v-model="item.book_out_num_type"
                          rules="required"
                    ...
                    <div class="group-input">
                      <div class="name">หน่วยงานปลายทาง</div>
                      <cpn-input-tags
                        v-model="item.department_dest_id"
                        :flagSearch="true"
                        :duplicate="true"
                ...
                <div class="d-flex justify-content-center">
                  <button
                    type="button"
                    class="add-register"
                    @click="add_booking_register_details_modal()"
                  >
                    <img
                      src="@/assets/images/icon/plus-circle-duotone.svg"
                      alt=""
              ...
              <div class="group-footer">
                <button
                  type="button"
                  @click="modalRegiter.showModal = false"
          ...
          <div class="modal-size" ref="modalSendref">
            ...
            <div class="modal-detail">
              <div class="group-head">
                ...
                <div class="group-input w-25">
                  ...
                  <cpn-select
                    v-model="modalSend.send_style_id"
                    name="send_style_id"
              ...
              <div
                class="message"
                v-for="(item, index) in modalSend.booking_register_details.filter(
                  (el) => el.flag != 'delete'
                )"
                :key="index"
              >
                ...
                <div class="detail-sub">
                  ...
                  <div
                    ...
                    v-for="(item2, index2) in item.booking_registers.filter(
                      (el) => el.flag != 'delete'
                    )"
                    :key="index2"
                  >
                    ...
                    <div class="group-input">
                      <cpn-input
                        v-model="item2.department_dest_name"
                        :disabled="true"
            ...
            <div class="group-footer">
              ...
              <button
                ...
                :disabled="!modalSend.send_style_id"
                @click="sendMailClick()"
              >
                <div class="group-name">
                  <img
        ...
        <div class="modal-center">
          <div class="modal-size" ref="modalNumberref">
            ...
            <div class="modal-detail">
              ...
              <div
                v-for="(item, index) in data.booking_register_details.filter(
                  (el) => el.flag != 'delete'
                )"
                :key="index"
              >
            ...
            <div class="group-footer">
              <button
                type="button"
                @click="modalNumber.showModal = false"
                class="btn button-danger"
              >
                <div class="group-name">
                  <img
    ...
    <div class="detail-history" v-if="$route.params.id">
      <div class="history">
        <div
          ...
          @click="(data.history.hide = !data.history.hide), historyClick(data.history.tab)"
        >
        ...
        <div class="content" v-show="data.history.hide">
          <div class="content-head">
            <div
              class="pointer"
              :class="data.history.tab == 1 ? 'active' : ''"
              @click="(data.history.tab = 1), historyClick(1)"
            >
              <i class="bi bi-border-all icon-size"></i>ทั้งหมด
            </div>
            <div
              class="pointer"
              :class="data.history.tab == 2 ? 'active' : ''"
              @click="(data.history.tab = 2), historyClick(2)"
            >
              <i class="bi bi-chat-left icon-size"></i>ความเห็นคำสั่ง
            </div>
            <div
              class="pointer"
              :class="data.history.tab == 3 ? 'active' : ''"
              @click="(data.history.tab = 3), historyClick(3)"
            >
              <i class="bi bi-pencil-square icon-size"></i>แก้ไขข้อมูล
            </div>
          </div>
          <div
            v-if="
              data.history.data.filter((el) =>
                data.history.tab == 2
                  ? el.type == 2
                  : data.history.tab == 3
                  ? el.type == 0 || el.type == 1
          ...
          >
            <div
              ...
              v-for="(item, index) in data.history.data.filter((el) =>
                data.history.tab == 2
                  ? el.type == 2
                  : data.history.tab == 3
                  ? el.type == 0 || el.type == 1
              ...
              :key="index"
              :class="index == 0 ? 'first' : index == data.history.data.length - 1 ? 'end' : ''"
            >
              <div class="detail-head">
                ...
                <div
                  class="topic"
                  :class="
                    item.bookactionname == 'ความเห็นคำสั่ง'
                      ? 'blue'
                      : item.bookactionname == 'แก้ไขหนังสือ'
                      ? 'yellow'
                      : 'green'
                  "
                >
                ...
                <div class="date">วันที่ {{ item.createDate }}</div>
                <div class="time">
                  <i class="bi bi-clock icon-size"></i>
                  {{ item.createTime }}
                </div>
              ...
              <ul class="detail-list" v-for="(item2, index2) in item.bookingRemarks" :key="index2">
                <button
                  v-show="item2.filepath"
                  class="button-file"
                  @click="
                    download_file({ filename: item2.filepath.split('/').pop(), link: item2.link })
                  "
                >
                  {{ item2.filepath.split("/").pop() }}
                </button>
                <li>
                  {{ item2.remark }}
                  {{ item2.comment }}
                  <div
                    class="detail-signager"
                    v-if="item2.signature_img && item.bookactionname == 'ความเห็นคำสั่ง'"
                  >
...
export default {
  ...
  data() {
    return {
      ...
      optionSelectDefault: {
        ...
        regis_id: [],
        book_out_num_type: [
          { name: "เลขเดียว", value: "0" },
          { name: "หลายเลข", value: "1" },
        ],
        send_method_id: [
          { name: "ตอบรับ", value: "0" },
          { name: "แจ้งเพื่อทราบ", value: "1" },
          { name: "ไม่ระบุ", value: "2" },
        ],
      },
      showLoading: false,
      edit: false,
      ...
      optionSelect: {
        creater_id: [],
        book_category_id: [
          { name: "บันทึกข้อความเสนอ นรม./รอง นรม.", value: "1" },
          { name: "หนังสือส่งออกภายนอก", value: "2" },
        ],
        book_type_id: [],
        secret_id: [],
        speed_id: [],
      ...
      modalSend: {
        ...
        optionSelect: {
          send_style: [
            { value: 1, name: "Email" },
            { value: 2, name: "ไปรษณีย์" },
            { value: 3, name: "รถนำส่ง" },
            { value: 4, name: "รับด้วยตนเอง" },
            { value: 5, name: "เจ้าของเรื่องส่งเอง" },
            { value: 6, name: "ส่งผ่านระบบสารบรรณ" },
            { value: 7, name: "ไม่ระบุ" },
          ],
        },
      },
      modalNumber: {
  ...
  methods: {
    check_all_gen_number(item) {
      let show = false;
      item.booking_registers
        .filter((el) => el.flag != "delete")
        .filter((row2) => {
    ...
    check_modal_number() {
      let show = false;
      this.data.booking_register_details
        .filter((el) => el.flag != "delete")
        .filter((row) => {
          row.booking_registers
            .filter((el) => el.flag != "delete")
            .filter((row2) => {
              if (!row2.is_real_book_out_num) {
                show = true;
    ...
    async sendNumberClick() {
      for (
        let i = 0;
        i < this.data.booking_register_details.filter((el) => el.flag != "delete").length;
        i++
      ) {
        ...
        if (row.select) {
          if (row.book_out_num_type != 1) {
            ...
            await this.axios
              .post(`/booking-out/generate-number`, {
            ...
            for (
              let i2 = 0;
              i2 < row.booking_registers.filter((el) => el.flag != "delete").length;
              i2++
            ) {
    ...
    sendMailClick() {
      ...
      this.modalSend.booking_register_details.filter((row, index) => {
        row.booking_registers.filter((row2, index2) => {
          if (
            row2.select &&
            (row2.book_id || row2.book_id == 0) &&
            (row2.regis_id || row2.regis_id == 0)
          ) {
            row2.send_style_id = this.modalSend.send_style_id;
            this.data.booking_register_details[index].booking_registers[index2].send_style_id =
              this.modalSend.send_style_id;
            this.data.booking_register_details[index].booking_registers[index2].send_style_desc =
              this.modalSend.optionSelect.send_style.filter(
                (row) => row.value == this.modalSend.send_style_id
              )[0]?.name || "";
            let dataSave = {
    ...
    historyClick(data) {
      this.showLoading = true;
      this.axios
        .get(`/booking-out/${this.$route.params.id}/history`, {
          params: {
            book_type: 1,
            department_id: parseInt(localStorage.getItem("department_id")),
          },
          ...
          this.data.history.data.filter((item, index) => {
            item.bookingRemarks.filter((item2, index2) => {
              ...
              if (item2.signature_img) {
                this.axios({
                  method: "get",
                  url: this.backendport + "/" + item2.signature_img,
                  baseURL: "",
                  responseType: "blob",
                })
                  .then((response3) => {
                    const blob = new Blob([response3.data], {
                      type: this.assetsUtils.getTypeFile(item2.signature_img),
                    });
                    item2.signature_img = URL.createObjectURL(blob);
                  })
                  .catch((error) => {
                    item2.signature_img = new URL(
                      `@/assets/images/default/signature_img.jpg`,
                      import.meta.url
                    ).href;
                  });
              } else {
    ...
    delete_click() {
      ...
      this.modalAlert = {
        ...
        afterPressAgree() {
          ...
          _this.axios
            .delete(`/booking-out/${_this.$route.params.id}`)
            .then(() => {
    ...
    keyup_send_to(e) {
      ...
      this.axios
        .get("/master-data/department-user/booking-out/subministry", {
          params: {
            keyword: e.target.value,
    ...
    delete_booking_refers(item, index) {
      ...
      if (
        this.data.booking_refers.length -
          this.data.booking_refers.filter((item) => item.flag == "delete").length <
        1
      ) {
      ...
      for (let i = 0; i < item.num; i++) {
        if (item.book_out_num_type == 0) {
          if (item.booking_registers.filter((el) => el.flag != "delete").length < 1) {
            this.showLoading = true;
            await this.axios
              .post(`/booking-out/generate-number-draft`, {
                department_id: parseInt(localStorage.getItem("department_id")),
                year: this.assetsUtils.currentDate().split("/")[2] - 543,
                user_id: parseInt(localStorage.getItem("user_id")),
              })
    ...
    delete_attachments(item, index) {
      ...
      if (
        this.data.attachments.length -
          this.data.attachments.filter((item) => item.flag == "delete").length <
        1
      ) {
    ...
    add_booking_follows() {
      this.data.sendTo.filter((item) => {
        if (
          !this.data.booking_follows.some(
            (el) => el.department_id === item.value && el.flag != "delete"
          )
        ) {
          let data = {
            ...item,
          ...
          this.optionSelect.process_type_id.find((item) => {
            if (item.value == this.data.process_type_id) {
              data.process_type_name = item.name;
            }
          });
          this.optionSelect.permission_id.find((item) => {
            if (item.value == this.data.permission_id) {
              data.permission_name = item.name;
            }
          });
      ...
      for (var i = 0; i < document.querySelector(`[name="${data}"]`).files.length; i++) {
        ...
        if (name == "main_docs") {
          if (file.type == "application/pdf") {
            ...
            this.data.booking_register_details[index] = {
              ...this.data.booking_register_details[index],
              ...dataFile,
            };
            this.data.booking_register_details[index].booking_registers.filter((item, index2) => {
              this.data.booking_register_details[index].booking_registers[index2] = {
                ...item,
                ...dataFile,
              };
            });
    ...
    async on_submit_modal() {
      for (let i = 0; i < this.modalRegiter.booking_register_details.length; i++) {
        ...
        item.optionSelect.regis_id.find((item2) => {
          if (item2.value == item.regis_id) {
            regis_name = item2.name;
          }
        });
        item.optionSelect.book_out_num_type.find((item2) => {
          if (item2.value == item.book_out_num_type) {
            book_out_num_type_name = item2.name;
          }
        });
        item.optionSelect.send_method_id.find((item2) => {
          if (item2.value == item.send_method_id) {
            send_method_name = item2.name;
          }
        });
        let data = {
          optionSelect: {
    ...
    upload_file_all() {
      ...
      this.data.attachments.filter((item) => {
        if (item.file) {
          ...
          axiosArray1.push(
            this.axios.post(`/upload/single`, formDataFile, {
            ...
            this.axios.spread((...responses) => {
              responses.filter((item, index) => {
                fileAttachments.push({
                  ...this.data.attachments.filter((el) => el.file)[index],
                  ...item.data.data,
                  filepath: item.data.data.path,
                });
              });
          ...
          .then(
            this.axios.spread((...responses) => {
              responses.filter((item, index) => {
                ...
                fileSendTo.push({
                  ...this.data.booking_follows[index],
                  ...item.data.data,
                  filepath: item.data.data.path,
                });
              });
    ...
    call_api_save(data) {
      ...
      let dataSave = {
        ...
        creater_id: this.data.creater_id
          ? parseInt(this.data.creater_id)
        ...
        booking_register_details: this.data.booking_register_details.filter((item) => {
          ...
          item.booking_registers.filter((item2) => {
            ...
            item2.department_dest_id = item2.department_dest_id
              ? parseInt(item2.department_dest_id)
              : null;
      ...
      this.showLoading = true;
      this.axios[this.edit ? "put" : "post"](
        `/booking-out${this.edit ? "/" + this.$route.params.id : ""}`,
        dataSave
      )
        .then((response) => {
          this.showLoading = false;
          if (this.flagSave != 4 && this.flagSave != 5 && this.flagSave != 6) {
            this.modalAlert = {
              ...
              title:
                this.flagSave == 1
                  ? "ทำการบันทึกแบบร่างสำเร็จแล้ว"
                  : this.flagSave == 3
                  ? "ทำการบันทึกสำเร็จแล้ว"
                  : "ทำการบันทึกและส่งต่อสำเร็จแล้ว",
              msgSuccess: true,
              afterPressAgree() {
                _this.checkPage(response);
              },
            };
          } else {
    ...
    api_detail(id) {
      this.showLoading = true;
      this.axios
        .get(`/booking-out/${id ? id : this.$route.params.id}`)
        .then((response) => {
          this.showLoading = false;
          ...
          response.data.data.booking_refers.filter((item) => {
            item.flag = "edit";
            this.axios
              .get(`/master-data/book-refer/${item.book_type}/${item.id}`)
              .then((response2) => {
          ...
          this.data.booking_register_details.filter((item) => {
            ...
            item.booking_registers.filter((item2) => {
              ...
              item2.flag = "edit";
              item2.main_link = item2.main_filepath
                ? this.backendport + "/" + item2.main_filepath
                : "";
              item2.attach_link = item2.attach_filepath
                ? this.backendport + "/" + item2.attach_filepath
                : "";
              item2.signer_id = item2.signer_id == 0 ? "" : item2.signer_id;
              item2.send_style_desc =
                this.modalSend.optionSelect.send_style.filter(
                  (row) => row.value == item2.send_style_id
                )[0]?.name || "";
              item2.department_dest_id =
                item2.department_dest_id == 0 ? "" : item2.department_dest_id;
              return item2;
            });
            return item;
          });
          ...
          if (this.data.attachments.length < 1 || !this.data.attachments)
            this.data.attachments = [{ filename: "", flag: "add" }];
        })
        .catch((error) => {
          this.showLoading = false;
          this.modalAlert = {
    ...
    api_master() {
      ...
      const request7 = this.axios.get(
        `/user/signer?user_id=${localStorage.getItem("user_id")}&department_id=${parseInt(
      ...
      this.axios
        ...
        .then(
          this.axios.spread((...responses) => {
            ...
            this.modalSend.optionSelect.send_style = response11.data.data;
