        ...
        <Form @submit="on_submit" @invalid-submit="onInvalidSubmit">
          <div class="group-head">
            <div class="group-first">
              ...
              <div class="name">
                {{ edit ? "แก้ไขบันทึกส่งออก" : "สร้างบันทึกส่งออก" }}
                <label>{{ data.booking_note_number }}</label>
              </div>
            </div>
            <button
              type="submit"
              class="add-booking-out"
              @click="flagSave = 3"
              v-show="!data.booking_note_number"
            >
              <div class="group-image">ออกเลขบันทึกภายใน</div>
            </button>
          </div>
          <div class="line"></div>
          <div class="group-detail">
            ...
            <div class="group-between">
              <div class="group-input">
                ...
                <cpn-input v-model="data.greeting" name="greeting" type="text" />
              </div>
            </div>
            <div class="group-between">
              <div class="group-input">
                <div class="name">รายละเอียด</div>
                <cpn-textArea v-model="data.desc" name="record_out_description" rows="1" />
              </div>
            </div>
            <div class="group-between">
              <div class="group-input">
                <div class="name">Tag สำหรับค้นหา</div>
                <cpn-input-tags v-model="data.tag" name="tag" placeholder="Enter a new tag" />
              </div>
            </div>
          ...
          <div class="group-detail">
            <div class="group-between">
              <div class="group-input left">
                ...
                <div
                  v-for="(item, index) in data.main_docs"
                  :key="index"
                  v-show="item.flag != 'delete'"
                >
                  <div class="d-flex mb-3">
                    <div class="group-input-file">
                      <button
                        type="button"
                        :disabled="data.is_disable"
                        class="button-file"
                        @click="upload_file(`main_docs${index}`)"
                      >
                      ...
                      <div
                        :class="data.is_disable ? 'text' : 'text pointer'"
                        @click="data.is_disable ? '' : upload_file(`main_docs${index}`)"
                      >
                        แนบเอกสาร
                      </div>
                      <input
                        type="file"
                        @change="file_set_change2(`main_docs${index}`, item, 'main_docs')"
                    ...
                    <button
                      ...
                      @click="delete_main_doc(item, index)"
                    >
                      <img
                        src="@/assets/images/icon/trash-alt-duotone.svg"
                        alt=""
              ...
              <div class="group-input">
                <div class="group-input d-flex align-items-center">
                  ...
                  <button
                    type="button"
                    class="add-booking-receive"
                    @click="upload_file('addAttachments')"
                  >
                    <div class="group-image">
                      <img
                        src="@/assets/images/icon/plus-circle-duotone.svg"
                  ...
                  <input
                    multiple
                    type="file"
                    @change="add_attachments_change('addAttachments')"
                    name="addAttachments"
                    style="display: none"
                  />
                </div>
                  ...
                  <div class="d-flex mb-3">
                    <div class="group-input-file">
                      <button
                        ...
                        :disabled="data.is_disable"
                        class="button-file"
                        @click="upload_file(`attachments${index}`)"
                      >
                        <span :class="item.filename ? '' : 'no-data'">
                      ...
                      <div
                        :class="data.is_disable ? 'text' : 'text pointer'"
                        @click="data.is_disable ? '' : upload_file(`attachments${index}`)"
                      >
                        แนบเอกสาร
                      </div>
                      <input
                        type="file"
                        @change="file_set_change2(`attachments${index}`, item, 'attachments')"
            ...
            <div class="group-input left">
              <div class="name d-flex justify-content-between">
                ...
                <div>
                  <input
                    type="file"
                    @change="file_set_change1('sendTo', 0, 'sendTo')"
                    name="sendTo"
                    style="display: none"
                  />
                  <button
          ...
          <div class="content" v-show="data.history.hide">
            ...
            >
              ...
              >
                ...
                <ul
                  class="detail-list"
                  v-for="(item2, index2) in item.bookingRemarks"
                  :key="index2"
                >
                  <button
...
export default {
  ...
  methods: {
    historyClick(data) {
      ...
      this.axios
        .get(`/booking-note/history/${this.$route.params.id}`, {
          params: {
            book_type: 4,
          },
        })
        .then((response) => {
          ...
          response.data.data.filter((item) => {
            item.bookingRemarks.filter((item2) => {
              item2.signature_img = item2.signature_img
                ? item2.signature_img
                : new URL(`@/assets/images/default/signature_img.jpg`, import.meta.url).href;
    ...
    delete_click() {
      ...
      this.modalAlert = {
        ...
        afterPressAgree() {
          ...
          _this.axios
            .delete(`/booking-note/${_this.$route.params.id}`)
            .then(() => {
    ...
    keyup_send_to(e) {
      ...
      this.axios
        .get("master-data/department-user/booking-note-out", {
          params: {
    ...
    delete_main_doc(item, index) {
      ...
      if (
        this.data.main_docs.length -
          this.data.main_docs.filter((item) => item.flag == "delete").length <
        1
      ) {
      ...
      this.modalAlert = {
        ...
        afterPressAgree() {
          if (_this.flagSave == 3) {
            _this.showLoading = true;
            _this.axios
              .post(`/booking-note/generate-number`, {
                department_id: parseInt(localStorage.getItem("department_id")),
                creater_id: _this.data.creater_id
                  ? parseInt(_this.data.creater_id)
                  : parseInt(localStorage.getItem("user_id")),
                book_category_id: parseInt(_this.data.book_category_id),
                year: _this.data.as_of_date.split("/")[2] - 543,
              })
              .then((response) => {
        ...
        this.axios
          ...
          .then(
            this.axios.spread((...responses) => {
              responses.filter((item, index) => {
                filemain_docs.push({
                  ...this.data.main_docs.filter((el) => el.file)[index],
                  ...item.data.data,
                  filepath: item.data.data.path,
                });
              });
    ...
    call_api_save(filemain_docs, file_attachments) {
      ...
      let dataSave = {
        ...
        flag:
          this.flagSave == 1 ? "draft" : this.flagSave == 3 || this.flagSave == 4 ? "update" : "",
      ...
      this.axios[this.edit ? "put" : "post"](
        `/booking-note${this.edit ? "/" + this.$route.params.id : ""}`,
        dataSave
      )
        .then(() => {
          this.showLoading = false;
          this.modalAlert = {
            ...
            title:
              this.flagSave == 1
                ...
                : this.flagSave == 3
                ? "ทำการออกเลขบันทึกภายในสำเร็จแล้ว"
                : this.flagSave == 4
    ...
    api_detail() {
      this.showLoading = true;
      this.axios
        .get(`/booking-note/${this.$route.params.id}`)
        .then((response) => {
          this.showLoading = false;
          ...
          if (this.data.main_docs.length < 1 || !this.data.main_docs)
            this.data.main_docs = [{ filename: "", flag: "add" }];
        })
        .catch((error) => {
          this.showLoading = false;
          this.modalAlert = {
    ...
    keyup_record_type(e) {
      this.optionSelect.sendTo = [];
      this.axios
        .get("/master-data/book-type-note", {
          params: {
            keyword: e.target.value,
          },
        })
